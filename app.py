...
def UnblockSite():
    if request.method == "POST":
        ...
        db = get_db()
        cursor = db.cursor()
        cursor.execute("INSERT INTO unblocked (site_name, unblock_time, unblock_date) VALUES (?, ?, ?)", (unblocked,formatted_time, current_date))
        db.commit()
...
@app.route('/block_entry')
def block_entry():
    conn = sqlite3.connect('websiteblock.db')
    cursor = conn.cursor()
...
@app.route('/unblock_entry')
def unblock_entry():
    conn = sqlite3.connect('websiteblock.db')
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM unblocked")
    unblocked_sites = cursor.fetchall()
    cursor.close()
    conn.close()
    return render_template('unblocked_sites.html', unblocked_sites=unblocked_sites)
